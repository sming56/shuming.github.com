...
function get_containerid_from_hostname()
{
    ...
    if [[ $result == "" ]]
    then
        ids=`ssh -i /home/shuming02/.ssh/id_rsa -t shuming02@$main_host sudo -iu sankuai sudo -s docker ps |grep -v pause | grep -v "prometheus-node" | awk '{print $1}'`
    else
...
do
        ...
        if [[ $result == "" ]]
        then
            ssh -i /home/shuming02/.ssh/id_rsa -t shuming02@$host1 "sudo -iu sankuai sudo -s  docker exec -it $cid_found /usr/bin/yum -y install iperf"
        else
            sudo -iu sankuai sudo -s  docker exec -it $cid_found /usr/bin/yum -y install iperf
        fi
	...
	#install iperf to the new created containers
        ...
        if [[ $result == "" ]]
        then
            ssh -i /home/shuming02/.ssh/id_rsa -t shuming02@$host2 "sudo -iu sankuai sudo -s  docker exec -it $cid_found /usr/bin/yum -y install iperf"
        else
            sudo -iu sankuai sudo -s  docker exec -it $cid_found /usr/bin/yum -y install iperf
        fi
...
do
        ...
        if [[ $result == "" ]]
        then
            ssh -i /home/shuming02/.ssh/id_rsa -t shuming02@$host1  sudo -iu sankuai  sudo -s  docker exec -itd $id1 nohup iperf -s
        else
            sudo -iu sankuai  sudo -s  docker exec -itd $id1 nohup iperf -s
        fi
        #sleep 15 seconds for qperf server to start
        sleep 1
        result=$(echo ${current_host}  | grep "${host2}")
        if [[ $result == "" ]]
        then
            ssh -i /home/shuming02/.ssh/id_rsa -t shuming02@$host2 sudo -iu sankuai sudo -s  docker exec -itd $id2 iperf -c $ip1 -t $interval
        else
            sudo -iu sankuai sudo -s  docker exec -itd $id2 iperf -c $ip1 -t $interval
        fi
done

#sleep a while for iperf to exit
sleep `expr $interval + 30`
#collect the output from iperf server
echo "collect iperf output from $actual_num servers"
echo container_hosts1=$container_hosts1
echo container_hosts2=$container_hosts2
touch all_outputs.txt
for id1 in $container_ids1
do
        echo collecting stats id=$id1
        result=$(echo ${current_host}  | grep "${host1}")
        if [[ $result == "" ]]
        then
            ssh -i /home/shuming02/.ssh/id_rsa -t shuming02@$host1 sudo -iu sankuai sudo -s docker exec -it $id1 cat /nohup.out > tmp_outputs.txt
        else
            sudo -iu sankuai sudo -s docker exec -it $id1 cat /nohup.out > tmp_outputs.txt
        fi
