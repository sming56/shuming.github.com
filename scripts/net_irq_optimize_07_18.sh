...
get_config()
{
   ...
   irq_log "hostCores: $hostCores"
   irq_log "hostLastCoreNum: $hostLastCoreNum"
...
#1.check user root and parameters
if [ $UID -ne 0 ]; then
  irq_log "please run this with root" 
  exit 1
fi

if [ ! -f $hulk_irq_check_file ]; then
	irq_log "error: /usr/bin/hulk_irq_cores_check.sh does not exist"
	exit 1
fi

if [ $# -lt "2" ]; then
	/usr/bin/hulk_irq_cores_check.sh
	if [ $? -eq "0" ]; then
		irq_log "irq check correct, no need to exec this script"
		exit 0
	fi
fi

#1.a tune nf_conntrack parameter
modprobe nf_conntrack
echo 655360 > /sys/module/nf_conntrack/parameters/hashsize
echo 2621440 > /proc/sys/net/nf_conntrack_max

#1.b make sure there is no container already running
#which docker  >/dev/null 2>&1
#if [ $? -eq 0 ]; then
#  systemctl status docker  >/dev/null 2>&1
#  if [ $? -eq 0 ]; then
#    con_num=$(docker ps|wc -l)
#    if [ $con_num -gt 1 ]; then
#      irq_log "container is already running, do not set irq affinity"
#      exit 1
#    fi
#  fi
#fi
...
if [ $# -eq "0" ]; then
	irq_log "need setting ring buffer"
	irq_log "begin to adjust nic's ringbuffer"
	for nic in $ALL_HOST_NICS
	do
  		irq_log "adjusting nic: $nic"
  		irq_log "`ifconfig $nic txqueuelen 5000 2>&1`"
		##下面这行暂且在ARM上注释掉，因为ARM机器华为网卡不兼容此设置
		if [ `uname -m` != "aarch64" ]; then
			rx_str=`ethtool -g $nic | grep RX: | awk -F: '{print $2}'| sed s/[[:space:]]//g`
            		tx_str=`ethtool -g $nic | grep TX: | awk -F: '{print $2}'| sed s/[[:space:]]//g`
            		rxs=($rx_str)
            		txs=($tx_str)
            		if [ ${rxs[0]} -ne ${rxs[1]} ] || [ ${txs[0]} -ne ${txs[1]} ]; then
                		irq_log "`ethtool -G $nic tx ${txs[0]} rx ${rxs[0]} 2>&1`"
            		else
                		irq_log "RX: ${rxs[0]} == ${rxs[1]}, not need to set"
                		irq_log "TX: ${txs[0]} == ${txs[1]}, not need to set"
            		fi
		fi
	done
elif [ $# -eq "1" ]; then
	case $1 in
		restore)
			irq_log "no need setting ring buffer"
			;;
		*)
			irq_log "command is not correct"
			irq_log "if the machine reboot:"
			irq_log "Usage: $0"
			irq_log ""
			irq_log "if you want check & restore:"
			irq_log "Usage: $0 restore"
			irq_log ""
			irq_log "if you want modify irq's core_num"
			irq_log "Usage: $0 modify NUM"
			exit 1
	esac
elif [ $# -eq "2" ];then
	case $1 in
		modify)
			irq_log "modfiy irq cores num to :$2"
			;;
		*)
			irq_log "command is not correct"
			irq_log "if the machine reboot:"
...
if [ "$autoSwitch" != "on" ]
then
    irq_log "autoSwitch is off,need to adjust irq affinity manually"
    exit 1
fi
